...
class BlindClassifier {
  ...
  private _train(dataset: TextClassifierType.Dataset, iteration = 0) {
    ...
    dataset.forEach((entry) => {
      ...
      layerized.forEach((token) => {
        ...
        if ("beta" in result && !!result.beta) {
          console.log("ENTER", result.beta);
          value[result.output] =
            (value[result.output] || 0) + result.beta !== Infinity
              ? result.beta
              : 1;
        } else if (result.output === -1) {
          console.log("NEW TYPE", result);
          value[this.blindCategory] = 1;
          this.blindCategory++;
        }
        console.log("MAX", this.blindCategory);
        for (let i = 0; i <= this.blindCategory; i++) {
  ...
  async loadModel(path: string): Promise<BlindClassifier> {
    try {
