...
class Markov {
  ...
  constructor(props: MarkovProperties) {
    ...
    this.props.input.forEach((e, i, a) => {
      ...
      let checkWordNotInStartWords = (refWord: any) => {
  ...
  private _makeChain(minLength: number) {
    try {
      let word = this.choice(this.startWords);
      let chain = [word];
      let iterations = 0;
      while (
        this.wordStats.hasOwnProperty(word.toLowerCase()) &&
   ...
   */
  makeChain(minLength: number = this.props.minLength || 10) {
    try {
      let chain = [];
      let iterations = 0;
      let cond1 = false;
